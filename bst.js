...
class BinarySearchTree {
	...
	insert(key, value) { //-------------------------------------
		...
		/* If the tree already exist, then start at the root, and compare it to the key you want to insert If the new key is less than the node's key then the new node needs to live in the left-hand branch.  */
		else if (key < this.key) {
			/* if the existing node does not have any left child, meaning that if the `left` pointer is empty then we can just instantiate and insert the new node as the left child of that node, passing `this` as the parent. */
			if (this.left == null) {
